...
class PoseChecker @Inject constructor(
    ...
    suspend fun check(
        ...
        suspendCoroutine<Either<Unit, String>> { continuation ->
            val image = try {
            ...
            val task = poseDetector.process(image)
                .addOnSuccessListener { pose ->
                    if (pose.allPoseLandmarks.isEmpty()) {
                        continuation.resume(
                            Either.Right(
                                context.getString(R.string.pose_detection_failed)
                            )
                        )
                    } else {
                        ...
                        if (rightShoulder != null
                            ...
                            if (eye == null) {
                                continuation.resume(
                                    Either.Right(
                                        context.getString(R.string.pose_detection_eyes_not_found)
                                    )
                                )
                            } else if (ankle == null) {
                                continuation.resume(
                                    Either.Right(
                                        context.getString(R.string.pose_detection_ankles_not_found)
                                    )
                                )
                            } else if (hip == null) {
                                continuation.resume(
                                    Either.Right(
                                        context.getString(R.string.pose_detection_hips_not_found)
                                    )
                                )
                            } else if (elbow == null) {
                                continuation.resume(
                                    Either.Right(
                                        context.getString(R.string.pose_detection_elbows_not_found)
                                    )
                                )
                            } else if (wrist == null) {
                                continuation.resume(
                                    Either.Right(
                                        context.getString(R.string.pose_detection_wrists_not_found)
                                    )
                                )
                            } else if (knee == null) {
                                continuation.resume(
                                    Either.Right(
                                        context.getString(R.string.pose_detection_knees_not_found)
                                    )
                                )
                            } else {
                                ...
                                if (eyeInFrame.not()
                                    ...
                                    continuation.resume(
                                        Either.Right(
                                            context.getString(R.string.pose_detection_missed_head_or_feet)
                                        )
                                    )
                                } else if (
                                    ...
                                    continuation.resume(
                                        Either.Right(
                                            context.getString(R.string.pose_detection_too_far_from_camera)
                                        )
                                    )
                                } else {
                                    ...
                                    if (subjectIsVertical.not()) {
                                        continuation.resume(
                                            Either.Right(
                                                context.getString(R.string.pose_detection_not_vertical_position)
                                            )
                                        )
                                    } else if (subjectIsNotUpsideDown.not()) {
                                        continuation.resume(
                                            Either.Right(
                                                context.getString(R.string.pose_detection_upside_down)
                                            )
                                        )
                                    } else {
                                        ...
                                        if (angleDifference < 6) {
                                            ...
                                            continuation.resume(
                                                Either.Right(
                                                    context.getString(R.string.pose_detection_arm_not_vertical_wirst_misaligned)
                                                )
                                            )
                                        } else if (distanceElbowHipX / height * 100f > 4f) {
                                            continuation.resume(
                                                Either.Right(
                                                    context.getString(R.string.pose_detection_arm_not_vertical_elbow_misaligned)
                                                )
                                            )
                                        } else {
                                            continuation.resume(
                                                Either.Right(
                                                    context.getString(R.string.pose_detection_arm_not_vertical)
                                                )
                                            )
                                        }
                                    }
                            ...
                            continuation.resume(
                                Either.Right(
                                    context.getString(R.string.pose_detection_shoulders_not_found)
                                )
                            )
                        }
                    }
